...
enum class VTrackerParam
{
    ...
    RECT_HEIGHT,
    /// Option for LOST mode: 0 - freeze, 1 - move by velocity (stop at edge),
    /// 2 - move by velocity (reset at edge).
    LOST_MODE_OPTION,
    ...
    TYPE,
    /// Custom parameter 1. CvTracker: object loss detection probability
    /// threshold (default 0.1).
    CUSTOM_1,
    /// Custom parameter 2. CvTracker: object re-capture detection probability
    /// threshold (default 0.4).
    CUSTOM_2,
    /// Custom parameter 3. CvTracker: pattern learning rate (default 0.075).
    CUSTOM_3,
    /// Enable EKF (CTRV) predictive filtering on tracker output. 0 - off
    /// (default, raw correlation peak), 1 - on (smoothed position +
    /// velocity, EKF-driven prediction during LOST/INERTIAL modes).
    ENABLE_EKF,
    /// Enable DNN embedding verifier: 0 - off (default), 1 - on. Requires a
    /// FeatureExtractor set via CvTracker::setFeatureExtractor(); without
    /// one the flag has no effect.
    ENABLE_DNN_VERIFIER,
    ...
    DNN_ACCEPT_THRESHOLD,
    /// Enable Global Motion Compensation: 0 - off (default), 1 - on.
    /// Whole-frame phase correlation measures the camera-induced image
    /// shift each frame and keeps the search position attached to the
    /// scene. Self-contained (no IMU), lens/FOV agnostic.
    ENABLE_GMC,
    /// Enable DNN multi-peak arbitration in TRACKING: 0 - off (default),
    /// 1 - on. When the correlation surface has several competing peaks,
    /// embed each candidate and lock onto the one that IS the object
    /// (highest bank similarity), not merely the brightest match.
    /// Requires the DNN verifier to be active.
    ENABLE_DNN_ARBITRATION,
    /// Enable DNN embedding re-acquisition in LOST: 0 - off (default),
    /// 1 - on. A cycling grid of crops across the whole search window is
    /// embedded and checked against the template bank; a match cues a
    /// local correlation detect for confirmation. Removes the feature-
    /// window "sight bubble" restriction on re-capture. Requires the DNN
    /// verifier to be active.
    ENABLE_DNN_REACQUISITION
};
...
public:
    ...
    /// Global Motion Compensation enable (see VTrackerParam::ENABLE_GMC).
    bool gmcEnabled{false};
    /// Last measured global scene shift, frame pixels. Set by video
    /// tracker (0 when GMC is off or the measurement was rejected).
    /// Log these to validate GMC against known camera motion.
    float gmcShiftX{0.0f};
    float gmcShiftY{0.0f};
    /// DNN multi-peak arbitration enable (see ENABLE_DNN_ARBITRATION).
    bool dnnArbitration{false};
    /// DNN embedding re-acquisition enable (see ENABLE_DNN_REACQUISITION).
    bool dnnReacquisition{false};
