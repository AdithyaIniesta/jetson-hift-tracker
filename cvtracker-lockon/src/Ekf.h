...
public:
    ...
    /// Shift the position state by a known, externally measured camera
    /// translation (global motion compensation). Velocity, heading and
    /// covariance are untouched: a deterministic reference-frame shift
    /// adds no state uncertainty and does not change object motion.
