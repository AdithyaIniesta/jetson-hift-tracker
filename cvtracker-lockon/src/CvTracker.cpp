...
// (damped). Sizes the box from appearance, no learned regression head. The
// go/no-go test showed DINOv2 cosine peaks sharply at the true size.
...
// Multi-peak arbitration (ENABLE_DNN_ARBITRATION). A secondary response
// peak is considered a competing candidate when it reaches this fraction
// of the primary peak; at most this many candidates are embedded; a
// non-primary candidate wins only when its bank similarity beats the
// primary's by at least the margin (prevents flip-flopping on ties).
constexpr float ARB_SECOND_PEAK_RATIO = 0.5f;
constexpr int ARB_MAX_CANDIDATES = 3;
constexpr float ARB_SIM_MARGIN = 0.05f;

// Embedding re-acquisition (ENABLE_DNN_REACQUISITION). Crops embedded per
// LOST frame while sweeping the search window; grid spacing as a factor
// of the rect size (crops are DNN_CROP_CONTEXT x rect, so 0.75 gives
// overlapping coverage).
constexpr int REACQ_CROPS_PER_FRAME = 4;
constexpr float REACQ_SPACING_FACTOR = 0.75f;
...
struct CvTracker::Impl
{
    ...
    std::vector<float> embBuf2;   // arbitration candidate buffer

    // Global Motion Compensation: whole-frame phase correlation removes
    // camera-induced image shift before any tracking logic runs.
    GlobalMotion gmc;

    // Embedding re-acquisition sweep cursor (LOST mode).
    int reacqIndex{0};
                          ...
                          std::vector<float>& emb, float scaleMul = 1.0f)
    {
        ...
        // while the scale probe follows the real size). Cropping without
        // the scale factor framed the object inconsistently between the
        // bank seed and later checks, decaying similarity on zoom /
        // altitude change (previously Known Limitation 7.3).
                           ...
                           int maxOut, float outDx[], float outDy[])
    {
        ...
        // Peaks already taken (window-center-relative integer coords).
        ...
        while (nOut < maxOut)
        {
            ...
            if (bx < 0)
                break;
            // Subpixel refinement (parabola), same as the core's peak.
    ...
    void processBufferedFrame(const BufFrame& f)
    {
        ...
        // Global Motion Compensation: measure the camera-induced scene
        // shift since the previous frame and move every stored screen
        // position with the scene, BEFORE any mode logic runs. The
        // matcher then only ever sees object-relative motion - a camera
        // jolt of e.g. 60 px stops being the tracker's problem.
        // STATIC keeps its literal screen-freeze semantics; FREE has no
        // position to maintain.
        params.gmcShiftX = 0.0f;
        params.gmcShiftY = 0.0f;
        if (params.gmcEnabled)
        {
            float gdx = 0.0f, gdy = 0.0f, gconf = 0.0f;
            if (gmc.measure(f.luma.data(), f.width, f.height, f.id, gdx,
                            gdy, gconf))
            {
                params.gmcShiftX = gdx;
                params.gmcShiftY = gdy;
                if (params.mode == MODE_TRACKING ||
                    params.mode == MODE_LOST ||
                    params.mode == MODE_INERTIAL)
                {
                    posX += gdx;
                    posY += gdy;
                    clampPos();
    ...
    void trackingStep(const BufFrame& f)
    {
        ...
        // back to the primary if no secondary qualifies — losing lock
        // is worse than a mild drift. TTL decrement runs unconditionally
        // so the constraint expires even when we don't use it.
        if (epiTtl > 0)
        {
            --epiTtl;
            ...
            if (norm2 > 1e-12f && epiHalfW > 0.0f)
            {
                ...
                if (primDist > epiHalfW)
                {
                    ...
                    {
                        const float sx = cx + sdx[i] * stepX * scale;
                        const float sy = cy + sdy[i] * stepY * scale;
                        const float d  = lineDist(sx, sy);
                    ...
                    // else: keep primary, drift will be corrected next
                    // frame when the peer's own correlation catches up.
                }
            }
        }

        // Multi-peak arbitration (optional): when the response surface
        // has competing peaks (clutter, a distractor entering the
        // window), the brightest match is not necessarily the object.
        // Embed each candidate and lock onto the one that IS the object
        // by bank similarity. Runs only on genuinely ambiguous frames.
        if (params.dnnArbitration && extractor && !bank.empty())
        {
            float sdx[ARB_MAX_CANDIDATES];
            float sdy[ARB_MAX_CANDIDATES];
            const int nSec = findSecondaryPeaks(
            ...
            {
                float bestSim = -2.0f;
                if (computeEmbedding(f, cx + r.dx * stepX * scale,
                                     cy + r.dy * stepY * scale, embBuf))
                    bestSim = bank.similarity(embBuf);
                const float primSim = bestSim;
                float bestDx = r.dx, bestDy = r.dy;
                for (int i = 0; i < nSec; ++i)
                {
                    if (!computeEmbedding(f, cx + sdx[i] * stepX * scale,
                                          cy + sdy[i] * stepY * scale,
                                          embBuf2))
                        continue;
                    const float sim = bank.similarity(embBuf2);
                    if (sim > bestSim)
                    {
                        bestSim = sim;
                        bestDx = sdx[i];
                ...
                // A non-primary candidate wins only decisively: above the
                // accept bar AND clearly better than the primary.
                if (bestSim >= params.dnnAcceptThreshold &&
                    bestSim >= primSim + ARB_SIM_MARGIN)
                {
                    r.dx = bestDx;
                    r.dy = bestDy;
                    params.dnnSimilarity = std::max(0.0f, bestSim);
                }
            }
        }
        ...
        // Skipped when the DNN scale-pick is active (the verifier's appearance-
        // based sizing above supersedes this correlation-based probe, so the
        // two don't fight over `scale`).
    ...
    void lostStep(const BufFrame& f)
    {
        ...
        // Embedding re-acquisition (optional): the correlation features
        // are windowed to ~2.5x the rect around the extraction center, so
        // the plain detect above cannot see an object re-appearing
        // further out ("sight bubble"). Sweep a cycling grid of embedding
        // crops across the WHOLE search window; a bank match cues a local
        // correlation detect at that spot for confirmation. Identity
        // finds, correlation confirms.
        if (params.dnnReacquisition && params.dnnVerifierEnabled &&
            extractor && !bank.empty())
        {
            const float sw = (float)params.searchWindowWidth * scale;
            const float sh = (float)params.searchWindowHeight * scale;
            const float spacing = std::max(
                8.0f, REACQ_SPACING_FACTOR *
                          (float)std::max(params.rectWidth,
                                          params.rectHeight) *
                          scale);
            const int gx = std::max(1, (int)(sw / spacing));
            const int gy = std::max(1, (int)(sh / spacing));
            const int total = std::min(gx * gy, 64);
            for (int n = 0; n < REACQ_CROPS_PER_FRAME; ++n)
            {
                const int idx = reacqIndex++ % total;
                const float ox = ((idx % gx) + 0.5f) * (sw / gx) - 0.5f * sw;
                const float oy = ((idx / gx) + 0.5f) * (sh / gy) - 0.5f * sh;
                const float px = std::min(
                    std::max(cx + ox, 0.0f), (float)params.frameWidth - 1.0f);
                const float py = std::min(
                    std::max(cy + oy, 0.0f),
                    (float)params.frameHeight - 1.0f);
                if (!computeEmbedding(f, px, py, embBuf2))
                    break; // extractor failure - stop sweeping this frame
                const float sim = bank.similarity(embBuf2);
                // Cue loosely, verify strictly: grid crops are generally
                // OFF-CENTER on the object (worst case half a grid cell),
                // which depresses similarity even for a perfect identity
                // match. The sweep therefore only needs a relaxed cue
                // threshold (half the accept bar) to justify one cheap
                // confirming detect; that detect CENTERS the object, and
                // the acceptance path below re-verifies the embedding at
                // the centered position against the FULL accept
                // threshold - precision is unchanged.
                const float cueThreshold =
                    std::max(0.15f, 0.5f * params.dnnAcceptThreshold);
                if (sim < cueThreshold)
                    continue;
                // Identity hypothesis - confirm with correlation there.
                extractWindow(f, px, py, scale, window);
                CorrelationCore::Result rr = core.detect(window);
                if (rr.prob > r.prob)
                {
                    r = rr;
                    cx = px;
                ...
                break; // one confirmed hypothesis per frame is enough
            }
        }
        params.detectionProbability = r.prob;
        ...
        if (r.prob > recaptureThreshold())
        {
            ...
            // position and flicker TRACKING/LOST (previously Known
            // Limitation 7.2). Re-seed at the measurement instead;
            // velocity re-converges within a few frames.
            if (params.ekfEnabled)
            {
...
CvTracker::CvTracker() : m_impl(new Impl)
{
    ...
    m_impl->gmc.init(m_impl->fft.get());
}
...
bool CvTracker::setParam(VTrackerParam id, float value)
{
    ...
    case VTrackerParam::DNN_ACCEPT_THRESHOLD:
        ...
        return true;
    case VTrackerParam::ENABLE_GMC:
        p.gmcEnabled = value != 0.0f;
        if (!p.gmcEnabled)
        {
            m_impl->gmc.reset();
            p.gmcShiftX = 0.0f;
        ...
        return true;
    case VTrackerParam::ENABLE_DNN_ARBITRATION:
        p.dnnArbitration = value != 0.0f;
        return true;
    case VTrackerParam::ENABLE_DNN_REACQUISITION:
        p.dnnReacquisition = value != 0.0f;
        return true;
    default:
        return false;
    }
...
float CvTracker::getParam(VTrackerParam id)
{
    ...
    case VTrackerParam::ENABLE_GMC: return p.gmcEnabled ? 1.0f : 0.0f;
    case VTrackerParam::ENABLE_DNN_ARBITRATION: return p.dnnArbitration ? 1.0f : 0.0f;
    case VTrackerParam::ENABLE_DNN_REACQUISITION: return p.dnnReacquisition ? 1.0f : 0.0f;
    default: return -1.0f;
    }
}
