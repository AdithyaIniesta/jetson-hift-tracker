...
void maskToBits(const VTrackerParamsMask& m, bool bits[NUM_FIELDS])
{
    const bool src[NUM_FIELDS] = {
        ...
        m.dnnAcceptThreshold, m.dnnSimilarity, m.gmcEnabled, m.gmcShiftX,
        m.gmcShiftY, m.dnnArbitration, m.dnnReacquisition };
                            ...
                            VTrackerParamsMask* mask)
{
    ...
    w.putB(gmcEnabled, bits[i++]);
    w.putF(gmcShiftX, bits[i++]);
    w.putF(gmcShiftY, bits[i++]);
    w.putB(dnnArbitration, bits[i++]);
    w.putB(dnnReacquisition, bits[i++]);
    if (!w.ok)
        return false;
    size = w.pos;
    return true;
...
bool VTrackerParams::decode(uint8_t* data, int dataSize)
{
    ...
    r.getB(gmcEnabled, bits[i++]);
    r.getF(gmcShiftX, bits[i++]);
    r.getF(gmcShiftY, bits[i++]);
    r.getB(dnnArbitration, bits[i++]);
    r.getB(dnnReacquisition, bits[i++]);
    return r.ok;
}
