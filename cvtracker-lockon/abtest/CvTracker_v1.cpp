...
struct CvTracker::Impl
{
    ...
    void estimateObject()
    {
        ...
        // Restrict to a neighborhood of the rectangle (1.6x) around center.
        const float rw = baseRectW * scale / stepX; // rect in window pixels
        const float rh = baseRectH * scale / stepY;
        const int cx = fftW / 2;
        const int cy = fftH / 2;
        const int hx = std::min(fftW / 2 - 1, (int)(0.8f * rw));
        const int hy = std::min(fftH / 2 - 1, (int)(0.8f * rh));

        long sumX = 0, sumY = 0, cnt = 0;
        int minX = fftW, maxX = -1, minY = fftH, maxY = -1;
        ...
        if (cnt < 16)
        {
            // Not enough evidence: object assumed to fill the rectangle.
            params.objectX = params.rectX;
            params.objectY = params.rectY;
            params.objectWidth = (int)std::lround(baseRectW * scale);
            params.objectHeight = (int)std::lround(baseRectH * scale);
            return;
        }
        const float mx = (float)sumX / cnt;
        const float my = (float)sumY / cnt;
        params.objectX =
            (int)std::lround(posX + (mx - cx) * stepX * scale);
        params.objectY =
            (int)std::lround(posY + (my - cy) * stepY * scale);
        params.objectWidth =
            (int)std::lround((maxX - minX + 1) * stepX * scale);
        params.objectHeight =
            (int)std::lround((maxY - minY + 1) * stepY * scale);
    }

    void applyAutoAdjust()
    {
        if (adjustSizeOnce || params.rectAutoSize)
        {
            // Move rectangle size toward estimated object size.
            const float aw = adjustSizeOnce ? 1.0f : 0.1f;
            params.rectWidth = (int)std::lround(
                (1 - aw) * params.rectWidth + aw * params.objectWidth);
            params.rectHeight = (int)std::lround(
                (1 - aw) * params.rectHeight + aw * params.objectHeight);
            params.rectWidth = std::max(8, params.rectWidth);
            params.rectHeight = std::max(8, params.rectHeight);
            adjustSizeOnce = false;
        }
        if (adjustPositionOnce || params.rectAutoPosition)
        {
            const float aw = adjustPositionOnce ? 1.0f : 0.2f;
            posX = (1 - aw) * posX + aw * (float)params.objectX;
            posY = (1 - aw) * posY + aw * (float)params.objectY;
            clampPos();
