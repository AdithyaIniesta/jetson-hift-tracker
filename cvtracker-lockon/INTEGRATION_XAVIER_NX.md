...
# Optionally add the TensorRT DNN verifier backend (JetPack ships TensorRT):
cmake -B build -DCMAKE_BUILD_TYPE=Release -DCVTRACKER_WITH_CUDA=ON \
      -DCVTRACKER_WITH_TENSORRT=ON -DCMAKE_CUDA_ARCHITECTURES=72
...
## 5a. Optional: DNN appearance verifier (TensorRT)

With `-DCVTRACKER_WITH_TENSORRT=ON` the tracker can verify at a low cadence
that it is still following the captured object (drift / distractor
rejection), using an ONNX embedding model run through TensorRT:
...
auto fe = createTrtFeatureExtractor(cfg);          // builds + caches engine
if (fe)
{
...
First run builds the engine (minutes); it is cached next to the ONNX file
for instant reload. FP16 is used automatically. Budget ~1-3 ms per
verification on Xavier NX at the default 6-frame cadence. Without a model
you can still smoke-test the pipeline with the built-in fallback:
`tracker.setFeatureExtractor(std::make_shared<TinyPatchExtractor>());`.
