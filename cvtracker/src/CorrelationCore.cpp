...
void CorrelationCore::init(int w, int h, FftBackend* fft)
{
    ...
    m_Acap.assign(n, cf(0.0f, 0.0f));
    m_Bcap.assign(n, cf(0.0f, 0.0f));
    m_hasSnapshot = false;
    m_winFft.assign(n, cf(0.0f, 0.0f));
...
void CorrelationCore::setObjectSize(float rectW, float rectH)
{
    ...
    // PSR sidelobe statistics must exclude the trained response main lobe,
    // whose width follows the label sigma. A fixed +-5 px zone (legacy)
    // leaks main-lobe tails into the "sidelobe" mean/std for objects larger
    // than ~50 px, deflating PSR and with it the detection probability -
    // which suppressed legitimate re-captures. 2.5 sigma covers ~99% of
    // the lobe energy.
    m_exclX = std::min(m_w / 4, std::max(5, (int)std::lround(2.5f * sx)));
    m_exclY = std::min(m_h / 4, std::max(5, (int)std::lround(2.5f * sy)));
...
void CorrelationCore::capture(const std::vector<float>& window)
{
    ...
    // Snapshot the freshly trained filter. The adaptive copy decays toward
    // whatever occludes/replaces the object in the frames before a loss is
    // declared; the snapshot preserves the object as captured and is used
    // by detectBest() for LOST-mode re-detection.
    m_Acap = m_A;
    m_Bcap = m_B;
    m_hasSnapshot = true;
...
CorrelationCore::Result CorrelationCore::detect(const std::vector<float>& window)
{
    ...
    if (!m_initialized)
        return res;
    preprocess(window.data(), m_winFft);
    m_fft->fft2d(m_winFft.data(), m_w, m_h, false);
    return correlateWith(m_A, m_B);
}

CorrelationCore::Result CorrelationCore::detectBest(const std::vector<float>& window)
{
    Result res;
    if (!m_initialized)
        return res;
    preprocess(window.data(), m_winFft);
    m_fft->fft2d(m_winFft.data(), m_w, m_h, false);
    res = correlateWith(m_A, m_B);
    if (m_hasSnapshot)
    {
        // Same window FFT correlated against the capture-time filter: only
        // the pointwise multiply and the inverse FFT are repeated (~60% of
        // one detect), not the preprocessing / forward FFT.
        Result snap = correlateWith(m_Acap, m_Bcap);
        snap.snapshot = true;
        if (snap.prob > res.prob)
            res = snap;
    }
    return res;
}

void CorrelationCore::restoreSnapshot()
{
    if (!m_hasSnapshot)
        return;
    m_A = m_Acap;
...
CorrelationCore::Result CorrelationCore::correlateWith(const std::vector<cf>& A,
                                                       const std::vector<cf>& B)
{
    Result res;
    const size_t n = (size_t)m_w * m_h;
    for (size_t i = 0; i < n; ++i)
    {
        const cf hConj = A[i] / (B[i] + cf(m_lambda, 0.0f));
        m_buf[i] = m_winFft[i] * hConj;
    }
    ...
    // PSR: exclude the main-lobe region (2.5 sigma per axis) around the peak.
    double mean = 0.0;
    int cnt = 0;
    for (int y = 0; y < m_h; ++y)
    {
        for (int x = 0; x < m_w; ++x)
        {
            if (std::abs(x - px) <= m_exclX && std::abs(y - py) <= m_exclY)
                continue;
    ...
    // Cosine window attenuation at the peak: an object detected off-center
    // produced its response through attenuated features, so its raw peak
    // under-reports the match quality by a KNOWN factor. Compensate the
    // peak before converting to probability - this makes the probability
    // approximately position-invariant across the window, which is what
    // makes LOST-mode off-center probing able to clear the re-capture
    // threshold. Guards: only compensate clearly-present peaks (>= 0.08,
    // noise peaks during occlusion stay uncompensated) and cap the boost
    // at 1/0.15x; the PSR term is position-invariant already and provides
    // the second, independent factor.
    res.attn = std::min(std::max(m_hann[(size_t)py * m_w + px], 0.0f), 1.0f);
    float peakEff = peak;
    if (peak >= 0.08f)
        peakEff = peak / std::max(res.attn, 0.15f);
    ...
    const float peakTerm = std::min(std::max(peakEff * 1.6f, 0.0f), 1.0f);
    const float psrTerm =
