...
# UART is hardcoded per JetPack — this is run_jp5.sh, use it only on a
# JetPack 5 Jetson. On JetPack 6, launch run_jp6.sh instead. Autodetect
...
# Distance from the boresight camera's optical origin to the target
# plane, millimetres. Indoor: measure with a ruler (typical 2000 mm
# for a tabletop test). Blank / 0 disables handoff.
echo ""
...
if [ -n "$TARGET_DEPTH_MM" ] && [ "$TARGET_DEPTH_MM" != "0" ]; then
    # Match the resolution-specific naming the binary uses at load time.
